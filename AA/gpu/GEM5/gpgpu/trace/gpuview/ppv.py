...
def parse_trace_file(input_path, output_path, raw_output_path):
    ...
    while i < len(lines):
        ...
        if m:
            ...
            C.append(f"fetch:{fetchtick[5:11]},pc:{pc_raw}")
            C.append(f"decode:{decode[5:11]}")
            C.append(f"scb:{scb[5:11]}")
            C.append(f"sch:{sch[5:11]}")
            C.append(f"issue:{issue[5:11]}")
            if (macc == "0"):
                C.append(f"complete:{int(issue[5:11])+4}")
            else:
                C.append(f"macc:{macc[5:11]}")
                C.append(f"stlbReturn:{tlbR[5:11]}")
                C.append(f"reqScache:{reqCache[5:11]}")
                C.append(f"ScacheResp:{cacheResp[5:11]}")
                C.append(f"mc:{mc[5:11]}")
...
# 示例用法：
parse_trace_file("tracev2", "trace.out", "trace.raw")
